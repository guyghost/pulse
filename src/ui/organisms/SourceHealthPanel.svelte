            ...
            >
                <div class="flex items-center gap-1.5">
                    {#each sortedSources as source (source.connectorId)}
                        {@const missionCount =
                            scanResultCounts.get(source.connectorId) ?? 0}
                ...
                {hasData
                        ...
                        >
                            ...
                            {:else}
                                <span
                                    class="text-[8px] font-bold text-text-secondary"
                                >
            ...
            <div class="mt-2 space-y-0.5">
                {#each sources as source (source.connectorId)}
                    {@const missionCount =
                        scanResultCounts.get(source.connectorId) ?? 0}
                    {@const isFiltered =
                        activeSourceFilter === source.connectorId}
                    {@const isEnabled = enabledConnectors
                        ? enabledConnectors.has(source.connectorId)
                        : true}
                    <div
                  ...
                  {isEnabled
                                        ? 'translate-x-4 bg-accent-emerald'
                        ...
                        >
                            ...
                            {:else}
                                <span
                                    class="text-[9px] font-bold text-text-secondary"
                                >
                                    {source.name.slice(0, 2).toUpperCase()}
                                </span>
                            {/if}
                        ...
                        <!-- Name -->
                        <span
                            class="min-w-0 flex-1 truncate text-[11px] font-medium text-text-primary"
                        >
                            {source.name}
                        </span>
                  ...
                  {isFiltered
                                ...
                                onclick={() =>
                                    onFilterBySource?.(
                                        isFiltered ? null : source.connectorId,
                                    )}
                                title={isFiltered
                                    ? "Retirer le filtre"
                                    : `Filtrer par ${source.name}`}
                            >
                                {missionCount}
                            </button>
                        {/if}
                        ...
                        <div class="flex items-center gap-1.5">
                            ...
                            {:else if source.sessionStatus === "connected"}
                                ...
                                {/if}
                                <span
                                    class="flex items-center gap-1 text-[10px] text-accent-emerald"
                                >
                                    <span
                                        class="inline-block h-1.5 w-1.5 rounded-full bg-accent-emerald"
                                    ></span>
                                    <span>Connecté</span>
                                </span>
