<script lang="ts">
  ...
  let scanStats = $derived(() => {
    const scanDurations = allMetrics
    ...
    const dedupRatios = allMetrics
      .filter((m) => m.name === 'scan.dedup_ratio')
      .map((m) => m.value);
    const avgDedup = dedupRatios.length > 0
  ...
  let cacheStats = $derived(() => {
    ...
    const sizes = allMetrics
      .filter((m) => m.name === 'cache.memory.size')
      .map((m) => m.value);
  ...
  let missionsByConnector = $derived(() => {
...
{#if isOpen}
  <div class="fixed inset-0 z-50 bg-navy-900/95 overflow-auto">
    <!-- Header -->
    <div class="sticky top-0 flex items-center justify-between px-4 py-3 bg-navy-800 border-b border-navy-700">
      <div class="flex items-center gap-3">
        ...
        <div class="flex gap-1">
          {#each ['overview', 'scan', 'cache', 'timings', 'webvitals'] as tab}
            <button
              class="px-2 py-1 text-[10px] font-mono rounded transition-colors {activeTab === tab ? 'bg-accent-blue text-white' : 'bg-surface text-text-secondary hover:text-text-primary'}"
      ...
      <div class="flex items-center gap-2">
        <button
          class="px-2 py-1 text-[10px] font-mono rounded {autoRefresh ? 'bg-accent-emerald/20 text-accent-emerald' : 'bg-surface text-text-secondary'} transition-colors"
    ...
    <div class="p-4 max-w-6xl mx-auto">
      ...
      {#if activeTab === 'overview'}
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          ...
          <div class="bg-surface rounded p-3">
            <div class="text-[10px] uppercase text-text-secondary tracking-wider">Avg Scan Time</div>
            <div class="text-2xl font-mono text-accent-blue">{formatDuration(scanStats().avgScanTime)}</div>
          </div>
          <div class="bg-surface rounded p-3">
            <div class="text-[10px] uppercase text-text-secondary tracking-wider">Cache Hit Rate</div>
            <div class="text-2xl font-mono text-accent-emerald">{cacheStats().lastHitRate.toFixed(1)}%</div>
          </div>
          <div class="bg-surface rounded p-3">
            <div class="text-[10px] uppercase text-text-secondary tracking-wider">Dedup Ratio</div>
            <div class="text-2xl font-mono text-accent-amber">{scanStats().avgDedup.toFixed(1)}%</div>
          </div>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-4">
          ...
          <div class="bg-surface rounded p-3">
            <div class="text-[10px] uppercase text-text-secondary tracking-wider mb-2">Missions by Connector</div>
            ...
            {:else}
              <div class="space-y-1">
                {#each [...missionsByConnector()] as [connectorId, count]}
                  <div class="flex justify-between items-center py-1 border-b border-navy-700 last:border-0">
          ...
          <!-- Recent Errors -->
          <div class="bg-surface rounded p-3">
            <div class="text-[10px] uppercase text-text-secondary tracking-wider mb-2">Recent Errors</div>
            {#if recentErrors().length === 0}
              <div class="text-text-muted text-xs">No errors</div>
            {:else}
              <div class="space-y-1 max-h-40 overflow-auto">
                {#each recentErrors() as error}
                  <div class="flex justify-between items-center py-1 border-b border-navy-700 last:border-0 text-xs">
        ...
        <div class="space-y-4">
          <div class="bg-surface rounded p-4">
            <div class="grid grid-cols-3 gap-4">
              <div>
                <div class="text-[10px] uppercase text-text-secondary tracking-wider">Total Scans</div>
                <div class="text-xl font-mono text-text-primary">{scanStats().scanCount}</div>
              </div>
              <div>
                <div class="text-[10px] uppercase text-text-secondary tracking-wider">Avg Duration</div>
                <div class="text-xl font-mono text-accent-blue">{formatDuration(scanStats().avgScanTime)}</div>
              </div>
              <div>
                <div class="text-[10px] uppercase text-text-secondary tracking-wider">Last Duration</div>
                <div class="text-xl font-mono text-text-primary">{formatDuration(scanStats().lastScan)}</div>
              </div>
            </div>
          </div>
          ...
          <!-- All scan metrics -->
          <div class="bg-surface rounded p-3">
            <div class="text-[10px] uppercase text-text-secondary tracking-wider mb-2">Scan History</div>
            <div class="space-y-1 max-h-64 overflow-auto">
              {#each allMetrics.filter(m => m.name === 'scan.duration').reverse() as metric}
                {@const missions = allMetrics.find(m => m.name === 'scan.missions.total' && m.timestamp === metric.timestamp)}
                <div class="flex justify-between items-center py-1 border-b border-navy-700 last:border-0 text-xs">
                  <span class="font-mono text-text-secondary">{formatTimestamp(metric.timestamp)}</span>
      ...
      {#if activeTab === 'cache'}
        <div class="space-y-4">
          ...
          <!-- All cache metrics -->
          <div class="bg-surface rounded p-3">
            <div class="text-[10px] uppercase text-text-secondary tracking-wider mb-2">Cache History</div>
            <div class="space-y-1 max-h-64 overflow-auto">
              {#each allMetrics.filter(m => m.name === 'cache.memory.hit_rate').reverse() as metric}
                {@const hits = allMetrics.find(m => m.name === 'cache.memory.hits' && m.timestamp === metric.timestamp)}
                {@const misses = allMetrics.find(m => m.name === 'cache.memory.misses' && m.timestamp === metric.timestamp)}
                <div class="flex justify-between items-center py-1 border-b border-navy-700 last:border-0 text-xs">
                  <span class="font-mono text-text-secondary">{formatTimestamp(metric.timestamp)}</span>
                  <span class="text-accent-emerald">{metric.value.toFixed(1)}%</span>
                  <span class="text-text-muted">{hits?.value || 0} hits / {misses?.value || 0} misses</span>
                </div>
              {/each}
            </div>
          </div>
      ...
      {#if activeTab === 'timings'}
        <div class="bg-surface rounded p-3">
          <div class="text-[10px] uppercase text-text-secondary tracking-wider mb-2">Operation Timings</div>
          ...
          {:else}
            <div class="space-y-1">
              {#each [...avgTimings()].sort((a, b) => b[1].avg - a[1].avg) as [op, stats]}
                <div class="flex justify-between items-center py-2 border-b border-navy-700 last:border-0">
                  ...
                  <div class="flex items-center gap-4">
                    ...
                    <span class="text-xs font-mono text-accent-blue w-16 text-right">{stats.avg}ms avg</span>
                  </div>
                </div>
              {/each}
            </div>
      ...
      {#if activeTab === 'webvitals'}
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div class="bg-surface rounded p-4">
            ...
            <div class="text-2xl font-mono {webVitals.fcp < 1800 ? 'text-accent-emerald' : webVitals.fcp < 3000 ? 'text-accent-amber' : 'text-accent-red'}">
          ...
          <div class="bg-surface rounded p-4">
            ...
            <div class="text-2xl font-mono {webVitals.lcp < 2500 ? 'text-accent-emerald' : webVitals.lcp < 4000 ? 'text-accent-amber' : 'text-accent-red'}">
          ...
          <div class="bg-surface rounded p-4">
            ...
            <div class="text-2xl font-mono {webVitals.cls < 0.1 ? 'text-accent-emerald' : webVitals.cls < 0.25 ? 'text-accent-amber' : 'text-accent-red'}">
          ...
          <div class="bg-surface rounded p-4">
            ...
            <div class="text-2xl font-mono {webVitals.fid && webVitals.fid < 100 ? 'text-accent-emerald' : webVitals.fid && webVitals.fid < 300 ? 'text-accent-amber' : 'text-accent-red'}">
        ...
        <div class="bg-surface rounded p-3 mt-4">
          <div class="text-[10px] uppercase text-text-secondary tracking-wider mb-2">Web Vitals History</div>
          <div class="space-y-1 max-h-64 overflow-auto">
            {#each allMetrics.filter(m => m.name.startsWith('webvital.')).reverse() as metric}
              <div class="flex justify-between items-center py-1 border-b border-navy-700 last:border-0 text-xs">
                <span class="text-text-secondary">{metric.name.replace('webvital.', '').toUpperCase()}</span>
                <span class="font-mono text-text-secondary">{formatTimestamp(metric.timestamp)}</span>
                <span class="font-mono text-text-primary">
                  {metric.name === 'webvital.cls' ? metric.value.toFixed(3) : `${Math.round(metric.value)}ms`}
                </span>
              </div>
            {/each}
          </div>
