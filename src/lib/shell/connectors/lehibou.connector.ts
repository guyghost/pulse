...
function formatDuration(duration: number | null, unit: string | null): string | null {
  ...
  if (u.includes('week') || u.includes('semaine')) return `${duration} semaine${duration > 1 ? 's' : ''}`;
...
export class LeHibouConnector extends BaseConnector {
  ...
  protected get sessionCheckUrl() { return `${API_BASE}/freelancers/me`; }
    ...
    try {
      ...
      for (let page = 1; page <= MAX_PAGES; page++) {
        ...
        for (const m of data.missions) {
          allMissions.push(createMission({
            id: `lh-${m.id}`,
            title: m.title,
            client: null,
            ...
            tjm: m.tjm,
            location: m.city ? `${m.city.title}${m.city.country ? `, ${m.city.country.title}` : ''}` : null,
