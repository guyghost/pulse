...
describe('parseLeHibouHTML', () => {
  ...
  it('retourne un tableau vide pour du HTML sans cartes', () => {
    expect(parseLeHibouHTML('<html><body><p>Aucun resultat</p></body></html>', NOW, ID_PREFIX)).toEqual([]);
  });
  ...
  it('ignore les cartes sans titre', () => {
    ...
    const missions = parseLeHibouHTML(htmlSansTitre, NOW, ID_PREFIX);
  ...
  it('retourne tjm null si le prix est absent', () => {
    ...
    const missions = parseLeHibouHTML(htmlSansPrix, NOW, ID_PREFIX);
