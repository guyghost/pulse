...
async function txt(sel) {
  return await page.locator(sel).first().innerText().catch(() => null);
}
try {
  await gotoApp(page);
  out.feedCountBadge = await page.locator('[aria-label*="missions dans la liste"]').first().innerText().catch(() => null);
  ...
  out.presets = await page.locator('[aria-label="Presets métier du feed"] button').allInnerTexts().catch(() => []);
  out.cardBtnArias = await page.evaluate(() => {
    const card = document.querySelector('[data-testid="mission-feed"] [data-testid="mission-card"], [data-testid="mission-card"], article');
  ...
  out.scoreBucketBtns = await page.getByText(/Prioritaires|À comparer|À qualifier/, { exact: false }).allInnerTexts().catch(() => []);

  await navigate(page, 'applications');
  await page.waitForTimeout(500);
  out.appBody = (await page.locator('main, .flex.h-full').first().innerText().catch(() => '')).slice(0, 1400);

  await navigate(page, 'cv');
  await page.waitForTimeout(500);
  out.cvLinkedInBtns = await page.getByRole('button').filter({ hasText: /LinkedIn/i }).allInnerTexts().catch(() => []);
  ...
  await page.waitForTimeout(500);
  out.settingsButtons = await page.getByRole('button').allInnerTexts().catch(() => []);
} finally {
  await browser.close();
}
