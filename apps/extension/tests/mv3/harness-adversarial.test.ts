...
test.describe('packaged MV3 harness diagnostic gate', () => {
  test(
    'instruments a restarted service worker before its bootstrap executes',
    { annotation: { type: 'scenario-id', description: 'harness.bootstrap-diagnostics' } },
    async ({ extension }) => {
  ...
  test(
    'fails the test when the bootstrapping worker rejects a promise',
    { annotation: { type: 'scenario-id', description: 'harness.worker-rejection' } },
    async ({ extension }) => {
  ...
  test(
    'fails the test when the side panel raises an uncaught page error',
    { annotation: { type: 'scenario-id', description: 'harness.page-error' } },
    async ({ extension }) => {
  ...
  test(
    'fails the test when the side panel logs a console error',
    { annotation: { type: 'scenario-id', description: 'harness.page-console' } },
    async ({ extension }) => {
  ...
  test(
    'settles diagnostics before teardown so a late error cannot pass green',
    { annotation: { type: 'scenario-id', description: 'harness.late-diagnostic' } },
    async ({ extension }) => {
