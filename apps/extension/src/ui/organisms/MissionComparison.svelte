<script lang="ts">
  ...
  const fields: { label: string; key: string; render: (m: Mission) => string }[] = [
    ...
    { label: 'Remote', key: 'remote', render: (m) => (m.remote ? remoteLabels[m.remote] ?? m.remote : '—') },
...
{#if missions.length >= 2}
  <div class="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true">
    <div class="w-full max-w-lg animate-slide-up rounded-t-3xl bg-navy-800 border border-white/10 max-h-[85vh] overflow-y-auto">
      <!-- Header -->
      <div class="sticky top-0 z-10 flex items-center justify-between border-b border-white/8 bg-navy-800/95 backdrop-blur-sm px-4 py-3">
      ...
      <!-- Titles row -->
      <div class="grid border-b border-white/6 px-4 py-3" style="grid-template-columns: 90px repeat({missions.length}, 1fr)">
        <div class="text-[11px] uppercase tracking-[0.15em] text-text-muted self-end">Mission</div>
        {#each missions as mission}
          <div class="px-2">
            <a href={mission.url} target="_blank" rel="noopener" class="text-xs font-semibold text-accent-blue hover:underline line-clamp-2">
              {mission.title}
            </a>
          </div>
        {/each}
        ...
        {#each missions as mission}
          <div class="flex flex-wrap gap-1 px-2">
            {#each mission.stack.slice(0, 5) as tech}
              <span class="inline-flex rounded-full bg-accent-blue/10 px-1.5 py-0.5 text-[10px] text-accent-blue">{tech}</span>
            {/each}
      ...
      <!-- Actions -->
      <div class="grid px-4 py-3 border-t border-white/6" style="grid-template-columns: 90px repeat({missions.length}, 1fr)">
        <div></div>
        {#each missions as mission}
          <div class="px-2">
            <a
