...
<section class="section-card rounded-xl p-5">
  ...
  <div class="mt-4 grid gap-3">
    ...
    >
      <span>
        <span class="block text-meta font-medium text-text-primary">Activer les alertes</span>
        <span class="mt-0.5 block text-caption text-text-subtle">
    ...
    <div class="rounded-lg border border-border-light bg-page-canvas px-3 py-3">
      <div class="flex items-center justify-between gap-3">
        <div>
          <p class="text-meta font-medium text-text-primary">Pause temporaire</p>
          <p class="mt-0.5 text-caption leading-5 text-text-subtle">
      ...
      {#if isMuteActive}
        <p class="mt-2 rounded-md bg-surface-white px-2 py-1.5 text-caption text-text-subtle">
      ...
      <div class="mt-3 flex flex-wrap gap-2">
        <button
          type="button"
          class="inline-flex items-center gap-1.5 rounded-lg border border-border-light bg-surface-white px-2.5 py-1.5 text-caption font-medium text-text-primary transition-colors hover:bg-subtle-gray"
        ...
        <button
          type="button"
          class="inline-flex items-center gap-1.5 rounded-lg border border-border-light bg-surface-white px-2.5 py-1.5 text-caption font-medium text-text-primary transition-colors hover:bg-subtle-gray disabled:opacity-45"
    ...
    <div class="grid grid-cols-2 gap-2">
      <label class="rounded-lg border border-border-light bg-page-canvas px-3 py-2.5">
        ...
        <input
          ...
          class="mt-1 w-full bg-transparent font-mono text-body-lg font-semibold text-text-primary outline-none"
        />
      </label>
      <label class="rounded-lg border border-border-light bg-page-canvas px-3 py-2.5">
    ...
    <div class="rounded-lg border border-border-light bg-page-canvas px-3 py-3">
      <label for="alert-stack" class="text-meta font-medium text-text-primary"
        >Stacks requises</label
      >
      <div class="mt-2 flex gap-2">
        <input
          ...
          class="min-w-0 flex-1 rounded-lg border border-border-light bg-surface-white px-3 py-2 text-meta text-text-primary outline-none focus:border-blueprint-blue/30"
          onkeydown={(event) => {
            if (event.key === 'Enter') {
      ...
      {#if suggestedStacks.length > 0}
        <div class="mt-2 flex flex-wrap gap-1.5">
          {#each suggestedStacks.slice(0, 4) as stack (stack)}
            <button
              type="button"
              class="rounded-md bg-surface-white px-2 py-1 text-micro text-text-subtle transition-colors hover:text-text-primary"
  ...
  >
    <div class="flex items-start gap-2">
      ...
      <div class="min-w-0">
        <p class="text-meta font-medium text-text-primary">Aperçu avec vos données actuelles</p>
        <p class="mt-1 text-caption leading-5 text-text-subtle">{alertPreviewText}</p>
      </div>
    </div>
    ...
    {#if alertPreview.seenCount > 0}
      <p class="mt-2 text-micro leading-4 text-text-muted">{alertPreviewSeenText}</p>
    {/if}
  </div>
  ...
  <div class="mt-3 rounded-lg border border-border-light bg-page-canvas px-3 py-3">
    <div class="flex items-start justify-between gap-3">
      <div class="flex min-w-0 items-start gap-2">
        ...
        <div class="min-w-0">
          <p class="text-meta font-medium text-text-primary">Historique récent</p>
        ...
        {#each recentAlertHistory as entry, i (i)}
          <div class="rounded-lg border border-border-light bg-surface-white px-3 py-2">
            <div class="flex items-center justify-between gap-2">
              ...
              <span class="font-mono text-micro font-semibold text-text-primary">
                {entry.missionCount} mission{entry.missionCount > 1 ? 's' : ''}
              </span>
            </div>
            <p class="mt-1 truncate text-meta font-medium text-text-primary">
              {formatAlertHistoryTitles(entry)}
            </p>
            <p class="mt-1 truncate text-micro text-text-muted">
    ...
    {:else}
      <p class="mt-3 rounded-lg bg-surface-white px-3 py-2 text-caption leading-5 text-text-subtle">
  ...
  <button
    type="button"
    class="mt-4 inline-flex items-center gap-2 rounded-lg bg-blueprint-blue-strong px-3 py-2 text-meta font-medium text-white transition-colors hover:bg-blueprint-blue-strong/90 disabled:opacity-50"
    disabled={isSaving}
    onclick={save}
  >
