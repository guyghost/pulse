...
{#if isOpen}
  ...
  >
    ...
    >
      <div>
        <span class="text-meta font-bold text-blueprint-blue font-mono">DEV PANEL</span>
        <p class="mt-0.5 text-micro text-text-subtle">Centre de contrôle des scénarios locaux</p>
      </div>
      <button
    ...
    <div class="flex gap-1 px-3 pt-2 border-b border-border-light">
      <button
        class="px-3 py-1.5 text-caption font-mono rounded-t transition-colors {activeTab === 'main'
      ...
      <button
        class="px-3 py-1.5 text-caption font-mono rounded-t transition-colors {activeTab ===
        'metrics'
          ? 'bg-surface text-text-primary'
          : 'text-text-secondary hover:text-text-primary'}"
        onclick={() => {
    ...
    <div class="p-3 space-y-4">
      <section class="rounded-xl border p-3 {severityClasses(devScenario.severity)}">
        ...
        <p class="mt-1 text-body-lg font-semibold text-text-primary">{devScenario.title}</p>
        <p class="mt-1 text-meta leading-5 text-text-secondary">{devScenario.description}</p>
        <p class="mt-1 text-meta font-medium text-text-primary">{devScenario.action}</p>
      </section>
      ...
      <div>
        <div class="flex items-center justify-between gap-2">
          <span class="text-micro uppercase font-bold text-text-secondary tracking-wider"
            >Feed State</span
          >
          <span class="text-micro text-text-muted">Simuler une situation visible</span>
        </div>
        <div class="grid grid-cols-2 gap-1 mt-1 sm:flex">
          {#each [{ id: 'empty', label: 'empty', hint: 'Valider état vide' }, { id: 'loading', label: 'loading', hint: 'Valider skeleton' }, { id: 'loaded', label: 'loaded', hint: 'Valider feed actif' }, { id: 'error', label: 'error', hint: 'Valider incident' }] as state (state.id)}
            <button
              class="rounded-lg border border-border-light bg-surface-white px-2 py-1.5 text-left text-caption transition-colors hover:bg-subtle-gray"
            ...
            >
              ...
              <span class="block text-micro text-text-muted">{state.hint}</span>
            </button>
          {/each}
        </div>
      </div>
      ...
      <div>
        ...
        <div class="flex items-center gap-2 mt-1">
          ...
          />
          <span class="text-meta font-mono text-text-secondary w-10 text-right">{missionCount}</span
          >
          <button
      ...
      <div>
        <span class="text-micro uppercase font-bold text-text-secondary tracking-wider">Cache</span>
        <div class="mt-1">
          <button
            class="rounded-lg border border-border-light bg-surface-white px-2 py-1.5 text-caption font-medium text-text-secondary transition-colors hover:bg-subtle-gray hover:text-text-primary"
      ...
      <div>
        ...
        <div class="mt-1">
          <button
            class="w-full rounded-lg border border-status-violet/25 bg-status-violet/8 px-2 py-1.5 text-caption font-semibold text-status-violet transition-colors hover:bg-status-violet/12"
      ...
      <div>
        <div class="flex items-center justify-between gap-2">
          <span class="text-micro uppercase font-bold text-text-secondary tracking-wider"
            >Premium</span
          >
          <span class="text-micro text-text-muted">Scénario : {premiumScenario}</span>
        </div>
        <div class="grid grid-cols-3 gap-1 mt-1">
          ...
          >
            ...
            <span class="block text-micro text-text-muted">Tout ouvert</span>
          </button>
          <button
            class="rounded-lg border px-2 py-1.5 text-left text-caption transition-colors {premiumScenario ===
            'active-premium'
          ...
          >
            ...
            <span class="block text-micro text-text-muted">Gating live</span>
          </button>
          <button
            class="rounded-lg border px-2 py-1.5 text-left text-caption transition-colors {premiumScenario ===
            'active-free'
          ...
          >
            ...
            <span class="block text-micro text-text-muted">Paywall visible</span>
          </button>
        </div>
        <p class="mt-1 text-micro text-text-subtle">
        ...
        <div>
          <div class="flex items-center justify-between gap-2">
            <span class="text-micro uppercase font-bold text-text-secondary tracking-wider"
              >Bridge Logs</span
            >
            <span class="text-micro text-text-muted">Timeline des messages runtime</span>
          </div>
          <div
            class="mt-1 max-h-32 overflow-y-auto rounded-xl border border-border-light bg-surface-white p-2 font-mono text-micro space-y-0.5"
          >
            {#if logs.length === 0}
        ...
        <div class="space-y-3">
          <div class="grid grid-cols-3 gap-2">
            <div class="rounded-xl border border-border-light bg-surface-white p-2 text-center">
              <div class="text-micro uppercase text-text-secondary">Scans</div>
              <div class="text-heading font-mono text-text-primary">{scanMetrics.count}</div>
              <div class="text-micro text-text-muted">Historique</div>
            </div>
            <div class="rounded-xl border border-border-light bg-surface-white p-2 text-center">
              <div class="text-micro uppercase text-text-secondary">Latence</div>
              <div class="text-heading font-mono text-blueprint-blue">{scanMetrics.avgTime}ms</div>
              <div class="text-micro text-text-muted">À surveiller</div>
            </div>
            <div class="rounded-xl border border-border-light bg-surface-white p-2 text-center">
              <div class="text-micro uppercase text-text-secondary">Signaux</div>
              <div class="text-heading font-mono text-blueprint-blue">{scanMetrics.total}</div>
              <div class="text-micro text-text-muted">Session</div>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="flex gap-2 mt-1">
              <button
                class="flex-1 rounded-lg border border-blueprint-blue/25 bg-blueprint-blue/8 px-2 py-1.5 text-caption font-medium text-blueprint-blue transition-colors hover:bg-blueprint-blue/12"
                onclick={() => {
                  onExportMetrics?.();
                  metricsRefreshKey++;
                }}
          ...
          <p
            class="rounded-lg border border-blueprint-blue/15 bg-blueprint-blue/6 p-2 text-micro text-text-secondary"
          >
