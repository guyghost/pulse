...
The UI collection contains only `confirmed` records. `candidate` may be
previewed with a saving indicator, but cannot replace canonical state.
...
type ApplicationTrackingEvent =
  ...
  | { type: 'LOAD_SUCCEEDED'; mutationId: string; records: readonly MissionTracking[] }
  ...
  | { type: 'PERSIST_SUCCEEDED'; mutationId: string; tracking: MissionTracking | null }
  ...
  | { type: 'RECONCILED'; mutationId: string; tracking: MissionTracking | null };
```
...
stateDiagram-v2
  ...
  saving --> failed: PERSIST_FAILED [matchingMutation]
...
| `validRestore`     | Restore snapshot is null or validates as the same mission's complete tracking record.                                    |
...
| `ready`   | `REQUEST_RESTORE`        | valid       | `saving`    | Put previous snapshot or delete record atomically.                         |
| `saving`  | `PERSIST_SUCCEEDED`      | matching    | `ready`     | Publish confirmed record/delete; only now create success toast/Undo entry. |
| `saving`  | `PERSIST_FAILED`         | matching    | `failed`    | Retain previous canonical record and retry intent; show typed error.       |
...
Each `MissionTracking` record is atomic in the IndexedDB `mission_tracking`
store. `currentStatus`, history, generated assets, rating, notes, and
`nextActionAt` commit together. The mutation ID is included in the bridge
contract/idempotency record so duplicate delivery cannot append history twice.

UI candidate/previous/error state is ephemeral. Undo stores the confirmed
pre-mutation snapshot only after commit. On panel reload the database record is
canonical. On worker restart an unacknowledged UI mutation is reconciled by
reading the record; the UI never guesses whether the write committed.
...
7. Undo restores a confirmed snapshot and is itself a persisted mutation.
