...
export function createDebouncerWithCallback<T>(): {
	...
	checkCommit(state: DebounceState<T>, now: number): { shouldCommit: boolean; value?: T; newState: DebounceState<T> };
	/** Reset the debouncer */
	reset(): DebounceState<T>;
	/** Get initial state */
	getInitialState(): DebounceState<T>;
	...
	return {
		setValue(state: DebounceState<T>, value: T, delayMs: number, now: number) {
			const shouldCommitPrevious = state.pendingValue !== undefined && state.commitAt !== undefined && now < state.commitAt;
